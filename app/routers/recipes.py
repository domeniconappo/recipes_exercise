...
def _require_owner(recipe, current_user):
    if recipe.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not the recipe owner")
...
async def list_recipes(
    db: DB,
    vegetarian: Optional[bool] = Query(default=None, description="Filter by vegetarian status"),
    servings: Optional[int] = Query(default=None, ge=1, description="Filter by exact number of servings"),
    include_ingredients: Optional[List[str]] = Query(default=None, description="Include recipes with ALL of these ingredients"),
    exclude_ingredients: Optional[List[str]] = Query(default=None, description="Exclude recipes with ANY of these ingredients"),
    instructions_search: Optional[str] = Query(default=None, description="Case-insensitive text search within instructions"),
...
@router.post("", response_model=RecipeResponse, status_code=status.HTTP_201_CREATED, summary="Create a recipe")
...
@router.put("/{recipe_id}", response_model=RecipeResponse, summary="Update a recipe (partial)")
